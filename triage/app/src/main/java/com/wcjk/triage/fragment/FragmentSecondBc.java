 ...
 * 超声
 */
public class FragmentSecondBc extends FragmentBase {
    ...
    protected int setShowArray() {
        return R.array.secondarytriageultrasonic;
    }
    ...
    private void loadView(int num) {
        if (ll == null) return;
        if (num == 1) {
            View item = View.inflate(getActivity(), R.layout.second_bc_item1, null);
            LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
            ll.removeAllViews();
            ll.addView(item, lp);
        } else if (num == 2) {
            View item2_1 = View.inflate(getActivity(), R.layout.second_bc_item2, null);
            LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
            lp.weight = 1;
            ll.removeAllViews();
            ll.addView(item2_1, lp);
            View item2_2 = View.inflate(getActivity(), R.layout.second_bc_item2, null);
    ...
    private void bindClear() {
        ...
        for (int j = 0; j < ll.getChildCount(); j++) {
            ...
            TextView tv = (TextView) queue_view.findViewWithTag("queuename");
            if (tv != null) {
                tv.setText("");
            }
                ...
                while (it.hasNext()) {
                    ...
                    if (name.equals("patients")) {
                        ...
                        for (int i = 0; i < array.length(); i++) {
                            ...
                            if (!isFind && (row.getInt(status) == 2 || row.getInt(status) == 4
                                    || row.getInt(status) == 6 || row.getInt(status) == 8)) {
