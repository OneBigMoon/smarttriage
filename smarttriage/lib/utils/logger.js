...
const logger = module.exports = (filename) => {
  var extIndex;
  if (filename) {
...
logger.middleware = function koaLogger(logger4js, options) {
    ...
    if (req._logging) return next();
      ...
      //hook on end request to emit the log entry of the HTTP request.
			...
			if(res.statusCode && options.level === 'auto'){
				level = levels.TRACE;
				if(res.statusCode >= 400) level = levels.WARN;
				if(res.statusCode >= 500) level = levels.ERROR;
			} else {
				level = levels.toLevel(options.level, levels.TRACE);
			}
			if (thislogger.isLevelEnabled(level)) {
        let combined_tokens = assemble_tokens(req, res, options.tokens || []);
...
function assemble_tokens(req, res, custom_tokens) {
  ...
  default_tokens.push({ token: ':status', replacement: res.status });
  ...
  default_tokens.push({ token: ':remote-addr', replacement: req.headers['x-forwarded-for'] || req.ip ||
  ...
  default_tokens.push({ token: ':content-length', replacement: (res.header && res.header['content-length']) || '-' });
  default_tokens.push({ token: /:req\[([^\]]+)\]/g, replacement(_, field) {
    return req.headers[field.toLowerCase()];
  } });
  default_tokens.push({ token: /:res\[([^\]]+)\]/g, replacement(_, field) {
    return res.header  && res.header[field.toLowerCase()];
  } });
