...
module.exports = _.assign({}, util, {
  ...
  async transformPrimaryData(cache) {
    try {
  ...
  async transformSecondaryData(cache) {
    try {
      cache = JSON.parse(cache);
      ...
      result.patientcount = await redis.getValueFromRedis('secondary_count:' + result.doctorno, 'string');
      return result;
    } catch (err) {
      logger(__filename).error('transformSecondary error', err);
    }
  },
  async transformSecondarySplitData(caches) {
    try {
      const result = { queues: [] };
      for (let j = 0; j < caches.length; j++) {
        ...
        if (cache) {
          ...
          queue.patientcount = await redis.getValueFromRedis('secondary_count:' + queue.doctorno, 'string');
  ...
  async transformPacsSecondaryData(contents) {
    // var contents={ content:
    //   [ '[["超声9室","1100004","傅惠茹","6","10:40:03",null]]',
    //     '[["超声7室","1000011","于广玲","4","10:34:53",""]]',
    //     '[["超声3内","1000008","曾洁香","4","10:39:53","超声2号机"],["超声3内","C001","孙海风","0",null],["超声3内","C002","孙海风","0",null],["超声3内","C004","黄秋苹","0",null]]' ],
    //  consultingroomname: [ '超声9室name', '超声7室name', '超声3内name' ] }

    //  contents= { content: [ '[["超声9室","1100004","傅惠茹","6","10:40:03","超声9室"]]' ],
  //consultingroomname: [ '违反444' ] }

  
    try {
      let consultingroomname=contents.consultingroomname;
      let caches=contents.content;
      const result = { queues: [] };

      for(let i=0;i<caches.length;i++){

               if(caches[i]){
                caches[i]=JSON.parse(caches[i]);
                const queue = { patients: [] };
                
                for(let len=0;len<caches[i].length;len++){
                  const row = caches[i][len];
                  if(!queue.queuename && row[0]){
                    if(row[5]){
                      queue.queuename=row[5];

                        //如果有默认的
                    }else if(!row[5] && consultingroomname[i]){
  ...
  async transformDrawbloodData(cache) {
    try {
      cache = JSON.parse(cache);
      const result = {
        patiens: []
  ...
  async transformPharmacyData(cache) {
    try {
      cache = JSON.parse(cache);
      const result = {
        patiens: []
  ...
  async transformPharmacySecondaryData(cache, isMultiWin = false) {
    try {
      cache = JSON.parse(cache);
      const result = {
        waitPatients: [],
      ...
      for (let i = 0; i < cache.length; i++) {
        ...
        if (!result.winno) {
          result.winno = row[1];
          result.winname = row[8];
        }
        if (row[7] == '2' && row[6]) {
          result.callPatients.push({
            no: row[2],
            name: row[4].replace(/[0-9]/g, ''),
            namefull: row[3].replace(/[0-9]/g, ''),
            calltimes: row[5],
            winno: row[1],
            winname: row[8],
            calltime: row[6]
      ...
      result.callPatients = _.reverse(_.sortBy(result.callPatients, item => item.calltime));
      return result;
    } catch (err) {
      logger(__filename).error('transformPharmacySecondaryData error', err);
    }
  }
}, {
  crypto,
