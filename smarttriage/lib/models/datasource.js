...
module.exports = (Schema) => {
  ...
  const DatasourceSchema = new Schema({
    name: String, // 名称
    departmentid: [Number], // 一级分诊屏id ,多个用半角逗号分隔
    screenid: Number, // 屏id,二级his用
    screensplitid: [Number], // 屏id,二级his分屏
    queue: [String], // 队列名,pacs用,多个队列用半角逗号分隔
    consultingroomname: [String],//诊室名,pacs用,多个诊室名用半角逗号分隔
  ...
  DatasourceSchema
    ...
    .get(function() {
      ...
      if (this.queue) {
        ...
        if(this.consultingroomname){
            info.consultingroomname=this.consultingroomname.join(',');
        }
      }
      if (this.windowid) {
